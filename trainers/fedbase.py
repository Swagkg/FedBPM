...
class BaseFedarated(abc.ABC):
    ...
    def eval_on(self, round_i, clients, use_test_data=False, use_train_data=False, use_val_data=False):
        ...
        stats = {
            'epochs':round_i, 'mean acc': mean_acc, 'mean loss': mean_loss,
        }
        #
        if use_test_data:
    ...
    def save_model(self, round_i):
        self.worker.save(path=os.path.sep.join((self.metrics.resfedult_path, self.metrics.exp_name, f'model_at_round_{round_i}.pkl')))
